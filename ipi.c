...
//long (*sched_setaffinity)(pid_t pid, const struct cpumask *in_mask) = AFFINITY_ADDR;

void set_affinity(unsigned int cpu) {
	struct task_struct *m_task, *m_thread;
	for_each_process(m_task) {
		sched_setaffinity(m_task->pid, cpumask_of(cpu));
		list_for_each_entry(m_thread, &m_task->thread_group, thread_group) {
			sched_setaffinity(m_thread->pid, cpumask_of(cpu));
		}
	}
}
